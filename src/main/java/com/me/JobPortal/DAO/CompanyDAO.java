...
public class CompanyDAO extends DAO{
	...
	public void delete(Company comp) throws CompanyException{
		try {
//                        begin();
//			Query q = getSession().createQuery("delete from Company where companyId= :compId");
//			q.setInteger("compId", comp.getCompanyId());		
//			commit();


                        begin();
                        Query q = getSession().createQuery("from Company where companyId= :compId");
                        q.setParameter("compId", comp.getCompanyId());
                        Company companyToDelete = (Company) q.uniqueResult();
                        getSession().delete(companyToDelete);
                        commit();
